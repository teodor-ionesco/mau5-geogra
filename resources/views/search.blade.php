			...
			<div>
				<ol>
					@foreach($RESULTS['countries'] as $object)
						<li><a href="/countries/{{ $object -> code }}" target="">{{ strtoupper($object -> name) }}</a></li>
					@endforeach					
				</ol>
			</div>
			<br>
					...
					<tbody>
						@foreach($RESULTS['generalities'] as $object)
							<tr>
								...
								<td><a href="/countries/{{ $object -> country }}" target="">{{ strtoupper($object -> country) }}</a></td>
							</tr>
						@endforeach
					</tbody>
				</table>
				...
				<table class="table responsive-table bordered ">
					...
					<tbody>
						@foreach($RESULTS['sections'] as $object)
							<tr>
								<td><a href="/countries/{{ $object -> country }}/sections/{{ $object -> id }}" target="">{{ $object -> name }}</a></td>
								<td><a href="/countries/{{ $object -> country }}" target="">{{ strtoupper($object -> country) }}</a></td>
							</tr>
						@endforeach
					</tbody>
				...
				<table class="table highlight responsive-table bordered ">
					...
					<tbody>
						@foreach($RESULTS['theory'] as $object)
							<tr>
								<td><a href="/countries/{{ $object -> country }}/theory/{{ $object -> id }}" target="">{{ $object -> title }}</a></td>
								<td><a href="/countries/{{ $object -> country }}" target="">{{ strtoupper($object -> country) }}</a></td>
							</tr>
						@endforeach
					</tbody>
